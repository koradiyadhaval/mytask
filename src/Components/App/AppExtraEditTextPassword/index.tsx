...
export const AppExtraEditTextPassword = forwardRef(
  (props: CompProps, ref: any) => {
    const [showMask, setshowMask] = React.useState(props?.secureTextEntry);
    const GetRightView = () => {
      return (
        <>
          {!showMask &&
            props?.rightIconMask === true &&
            isNull({ data: props?.RightImageActivePath || "" }) && (
              <TouchableOpacity
                onPress={() => {
              ...
              >
                <Image
                  ...
                  resizeMode={"contain"}
                  style={[
                    styles.RightImageStyle,
                    props?.rightimagestyle,
                    {
                      tintColor:
                        isNull({ data: props.IsRightImageActivecolor }) === true
                          ? props.IsRightImageActivecolor
                          : undefined,
                    },
                  ]}
                  source={props?.RightImageActivePath}
                ></Image>
              </TouchableOpacity>
            )}
          ...
          {showMask &&
            props?.rightIconMask === true &&
            isNull({ data: props?.RightImageInActivePath }) && (
              <TouchableOpacity
                onPress={() => {
                  setshowMask(!showMask);
                }}
              ...
              >
                <Image
                  ...
                  style={[
                    ...
                    {
                      tintColor:
                        isNull({ data: props.IsRightImageInActivecolor }) ===
                        true
                          ? props.IsRightImageInActivecolor
                          : undefined,
                    },
                  ]}
                  source={props?.RightImageInActivePath}
                ></Image>
              </TouchableOpacity>
            )}
        </>
    ...
    return (
      <View style={[styles.ParentViews, props?.ParentViewStyle]}>
        {props?.IsLeftIcon &&
          isNull({ data: props?.leftImageUri }) === true && (
            <Image
              resizeMethod="resize"
              resizeMode="contain"
        ...
        <TextInput
          ...
          style={[
            ...
            props?.style,
            {
              letterSpacing: showMask
                ? deviceBasedDynamicDimension({ originalDimen: 2 })
                : deviceBasedDynamicDimension({ originalDimen: 3.5 }),
              flex: isNull(props?.RightImageActivePath) ? 0.9 : 1,
            },
          ]}
        >
          {/* {props?.children} */}
